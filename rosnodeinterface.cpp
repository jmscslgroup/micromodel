...
// Generated 09-Nov-2021 12:59:46
  ...
  namespace matlab
  {
    ...
    void NodeInterface::initialize(int argc, char ** argv)
    {
      try {
        ...
        ROS_INFO("** Starting the model \"micromodel\" **\n");

        // initialize the model which will initialize the publishers and subscribers
        rtmSetErrorStatus(micromodel_M, (NULL));
        micromodel_initialize();
    ...
    boolean_T NodeInterface::getStopRequestedFlag(void)
    {
      ...
      return (!(rtmGetErrorStatus(micromodel_M)
                == (NULL)));

#else

      return (!(rtmGetErrorStatus(micromodel_M)
                == (NULL)) || rtmGetStopRequested(micromodel_M));
    ...
    {
      mRunModel = (rtmGetErrorStatus(micromodel_M) ==
                   (NULL));
      while (mRunModel) {
        mBaseRateSem.wait();
